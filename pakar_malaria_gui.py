...
# Inisialisasi Prolog
prolog = Prolog()
prolog.consult("pakar_malaria_gui.pl")  # pastikan file .pl ada di folder yang sama
...
def pertanyaan_selanjutnya(ganti_penyakit=False):
    ...
    if index_gejala >= len(gejala[current_penyakit]):
        # Cek apakah diagnosa valid
        if list(prolog.query(f"diagnosa('{current_penyakit}')")):
...
kotak_pertanyaan = tk.Text(mainframe, height=4, width=50, state=tk.DISABLED, wrap=tk.WORD)
